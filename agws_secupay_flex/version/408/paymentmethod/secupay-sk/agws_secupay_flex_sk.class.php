...
class agws_secupay_flex_sk extends PaymentMethod
{
    ...
    public function agws_secupay_flex_getCurlContent($agws_linkaction, $agws_httpheader, $agws_data = null)
    {
        ...
        if (function_exists('curl_init')) {
            ...
            $agws_secupay_flex_logtext =
                'secupay_flex - SK - function getCurlContent <br />
    ...
    {
        $agws_secupay_flex_logtext =
        'secupay_flex - SK - function '.$agws_function.' <br />'.
    ...
    public function preparePaymentProcess($order)
    {
        ...
        if ($agws_secupay_flex_antwort_json->status == "ok" && isset($agws_secupay_flex_antwort_json->data->iframe_url) && isset($agws_secupay_flex_antwort_json->data->hash)) {
            ...
            if ($agws_secupay_flex_antwort_json->status == "ok" && isset($agws_secupay_flex_antwort_json->data->hash) && $agws_secupay_flex_antwort_json->data->hash == $_SESSION['agws_secupay_flex_hash_tmp']) {
                ...
                header("location: bestellvorgang.php?editZahlungsart=1&AGWS_SECUPAY_ZA=SK&AGWS_SECUPAY_ERRORCODE=".$agws_secupay_flex_antwort_json->status);
            }
        } else {
            //Logging + Redirect
    ...
    public function handleNotification($order, $agws_secupay_flex_paymentHash, $args)
    {
            ...
            if ($this->verifyNotification($order, $agws_secupay_flex_paymentHash, $args)) {
                ...
                $this->secupayHelper->isShop4()
                    ...
                    "INSERT INTO xplugin_agws_secupay_flex_tsyslog
					    ...
					    ('".Shop::DB()->realEscape($order->kBestellung)."', '".Shop::DB()->realEscape($_REQUEST['hash'])."',NOW(),'sofort')",
                        10
                    )
                    : $GLOBALS["DB"]->executeQuery(
                    "INSERT INTO xplugin_agws_secupay_flex_tsyslog
					(kBestellung, cHash, dSuccDat, cSecupayZA) VALUES
					('".$GLOBALS["DB"]->realEscape($order->kBestellung)."', '".$GLOBALS["DB"]->realEscape($_REQUEST['hash'])."', NOW(),'sofort')",
                        10
                    );
        ...
        if (isset($_REQUEST['sh']) && isset($_REQUEST['agws_sp_url']) && $_REQUEST['agws_sp_url']=='push') {
            ...
            if ($this->verifyNotification($order, $agws_secupay_flex_paymentHash, $args, $agws_secupay_flex_antwort_json)) {
                ...
                $agws_secupay_flex_Kommentar_Zweck = $smarty->fetch($this->oPlugin->cFrontendPfad . 'template/agws_secupay_flex_zweck_text_'.$_SESSION['cISOSprache'].'.tpl');
                ...
                if ($this->oPlugin->oPluginEinstellungAssoc_arr[$this->cModulId . "_agws_secupay_flex_sk_comment"] == 2) {
                    $this->secupayHelper->isShop4()
                        ? Shop::DB()->executeQuery("UPDATE tbestellung SET cPUIZahlungsdaten = '" . $agws_secupay_flex_Kommentar . "' WHERE kBestellung='".$order->kBestellung."'", 4)
                        : $GLOBALS['DB']->executeQuery("UPDATE tbestellung SET cPUIZahlungsdaten = '" . $agws_secupay_flex_Kommentar . "' WHERE kBestellung='".$order->kBestellung."'", 4);
                } else {
                    $this->secupayHelper->isShop4()
                ...
                $agws_secupay_flex_logtext =
                'secupay_flex - SK - function handleNotification <br />
        ...
        switch ($_REQUEST['agws_sp_url']) {
            case "succ":
                ...
                if ($status== true && (($_REQUEST['sh'] == $agws_secupay_flex_paymentHash) || ($_REQUEST['sh'] == "_".$agws_secupay_flex_paymentHash))) {
                    $agws_secupay_flex_logtext =
                        'secupay_flex - SK - function verifyNotification <br />
    ...
    public function isValid($customer, $cart)
    {
        ...
        if ($agws_secupay_flex_antwort_json->status == "ok" && in_array("sofort", $agws_secupay_flex_antwort_json->data)) {
