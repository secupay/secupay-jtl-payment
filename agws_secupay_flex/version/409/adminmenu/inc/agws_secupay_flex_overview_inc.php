...
function gibBestellungsUebersicht($cLimitSQL, $cSuchFilter, $cSecZA)
{
    ...
    ($helper->isShop4()) ? $oBestellungToday_arr = Shop::DB()
                                                       ->executeQuery($sql, 2)
    ...
    if (is_array($oBestellungToday_arr) && count($oBestellungToday_arr) > 0) {
        foreach ($oBestellungToday_arr as $oBestellungToday) {
            if (isset($oBestellungToday->kBestellung) && $oBestellungToday->kBestellung > 0) {
                ...
                $sql = "SELECT * FROM xplugin_agws_secupay_flex_tsyslog WHERE kBestellung = "
                    . $oBestellungToday->kBestellung . " ORDER BY kLogId DESC";
                ($helper->isShop4()) ? $oBestellungSecLog_arr = Shop::DB()
                                                                    ->executeQuery($sql, 8)
...
function gibAnzahlBestellungen($cSuchFilter, $cSecZA)
{
    ...
    ($helper->isShop4())
        ? $oBestellung = Shop::DB()
                             ->executeQuery($sql, 1) : $oBestellung = $GLOBALS['DB']->executeQuery(
        $sql,
        1
    );
...
function setzeAbgeholtZurueck($kBestellung_arr)
{
    ...
    if (is_array($kBestellung_arr) && count($kBestellung_arr) > 0) {
        ...
        $sql = "SELECT kKunde FROM tbestellung WHERE kBestellung IN(" . implode(",", $kBestellung_arr)
            . ") AND cAbgeholt = 'Y'";
        ($helper->isShop4())
            ? $oKunde_arr = Shop::DB()
                                ->executeQuery($sql, 2) : $oKunde_arr = $GLOBALS['DB']->executeQuery(
            $sql,
            2
        );
        ...
        if (is_array($oKunde_arr) && count($oKunde_arr) > 0) {
            ...
            ($helper->isShop4()) ? Shop::DB()
                                       ->executeQuery($sql, 3) : $GLOBALS['DB']->executeQuery($sql, 3);
        }

        // Bestellungen cAbgeholt zurücksetzen
        $sql = "UPDATE tbestellung SET cAbgeholt = 'N' WHERE kBestellung IN(" . implode(",", $kBestellung_arr)
            . ") AND cAbgeholt = 'Y'";
        ($helper->isShop4()) ? Shop::DB()
                                   ->executeQuery($sql, 3) : $GLOBALS['DB']->executeQuery($sql, 3);
