...
class agws_secupay_flex_kk extends PaymentMethod
{
    ...
    public function preparePaymentProcess($order)
    {
        ...
        if ($agws_secupay_flex_antwort_json->status == "ok" && isset($agws_secupay_flex_antwort_json->data->iframe_url)
            ...
            if ($agws_secupay_flex_antwort_json->status == "ok" && isset($agws_secupay_flex_antwort_json->data->hash)
                ...
                header(
                    "location: bestellvorgang.php?editZahlungsart=1&AGWS_SECUPAY_ZA=KK&AGWS_SECUPAY_ERRORCODE="
            ...
            header(
                "location: bestellvorgang.php?editZahlungsart=1&AGWS_SECUPAY_ZA=KK&AGWS_SECUPAY_ERRORMSG=" . urlencode(
                    utf8_decode($agws_secupay_flex_antwort_json->errors['0']->message)
                ) . "&AGWS_SECUPAY_ERRORCODE=" . urlencode(
                    utf8_decode($agws_secupay_flex_antwort_json->errors['0']->code)
                )
            );
        }
    }
    ...
    public function agws_secupay_flex_getCurlContent($agws_linkaction, $agws_httpheader, $agws_data = null)
    {
        ...
        if (function_exists('curl_init')) {
            ...
            $agws_secupay_flex_logtext = 'secupay_flex - KK - function getCurlContent <br />
    ...
    {
        $agws_secupay_flex_logtext = 'secupay_flex - KK - function ' . $agws_function . ' <br />' . $agws_logbase . '-'
            . $agws_logstatus . ' <br />
    ...
    public function handleNotification($order, $agws_secupay_flex_paymentHash, $args)
    {
        ...
        if (isset($_REQUEST['sh']) && isset($_REQUEST['agws_sp_url']) && $_REQUEST['agws_sp_url'] == 'push') {
            ...
            if ($this->verifyNotification(
                ...
                if ($this->oPlugin->oPluginEinstellungAssoc_arr[$this->cModulId . "_agws_secupay_flex_kk_comment"]
                    == 2) {
                    $this->secupayHelper->isShop4()
                        ? Shop::DB()
                              ->executeQuery(
                    ...
                    ? Shop::DB()
                          ->executeQuery(
                              ...
                              . "',kAmountSecupay='" . Shop::DB()
                                                           ->realEscape(
                                                               $agws_secupay_flex_antwort_json->data->amount
                                                           ) . "',dPushDat=now() WHERE kBestellung='"
                              . $order->kBestellung . "'",
                              4
                          )
                    : $GLOBALS["DB"]->executeQuery(
                ...
                $agws_secupay_flex_logtext = 'secupay_flex - KK - function handleNotification <br />
        ...
        switch ($_REQUEST['agws_sp_url']) {
            case "succ":
                ...
                if ($status == true
                    ...
                    $agws_secupay_flex_logtext = 'secupay_flex - KK - function verifyNotification <br />
