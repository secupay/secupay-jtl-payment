...
class agws_secupay_flex_rg extends PaymentMethod
{
    ...
    public function preparePaymentProcess($order)
    {
        ...
        $agws_secupay_flex_failURL     = $this->secupayHelper->gibShop__URL()
            . '/bestellvorgang.php?editZahlungsart=1&agws_url=fail';
        ...
        $agws_secupay_flex_daten = [
            'data' => [
                ...
                'firstname'        => utf8_encode(
                    html_entity_decode($agws_secupay_flex_customer->cVorname, ENT_COMPAT | ENT_HTML401, "ISO8859-1")
                ),
                'lastname'         => utf8_encode(
                    html_entity_decode($agws_secupay_flex_customer->cNachname, ENT_COMPAT | ENT_HTML401, "ISO8859-1")
                ),
                'street'           => utf8_encode(
                    html_entity_decode($agws_secupay_flex_customer->cStrasse, ENT_COMPAT | ENT_HTML401, "ISO8859-1")
                ),
                'housenumber'      => utf8_encode(
                    html_entity_decode($agws_secupay_flex_customer->cHausnummer, ENT_COMPAT | ENT_HTML401, "ISO8859-1")
                ),
                'zip'              => utf8_encode(
                    html_entity_decode($agws_secupay_flex_customer->cPLZ, ENT_COMPAT | ENT_HTML401, "ISO8859-1")
                ),
                'city'             => utf8_encode(
                    html_entity_decode($agws_secupay_flex_customer->cOrt, ENT_COMPAT | ENT_HTML401, "ISO8859-1")
                ),
                'country'          => utf8_encode(
                    html_entity_decode($agws_secupay_flex_customer->cLand, ENT_COMPAT | ENT_HTML401, "ISO8859-1")
                ),
                ...
                'order_id'         => '',
                'note'             => '',
                'module_config'    => array_map("utf8_encode", $this->oPlugin->oPluginEinstellungAssoc_arr),
                'apiversion'       => '2.3.14'
            ]
        ];
        ...
        $agws_secupay_flex_antwort      = $this->agws_secupay_flex_getCurlContent(
            'init',
            $agws_secupay_flex_http_header,
            $agws_secupay_flex_data
        );
        $agws_secupay_flex_antwort_json = json_decode($agws_secupay_flex_antwort);

        if ($agws_secupay_flex_antwort_json->status == "ok" && isset($agws_secupay_flex_antwort_json->data->iframe_url)
            && isset($agws_secupay_flex_antwort_json->data->hash)) {
            //Logging
            $this->agws_secupay_flex_doLoggingCurl(
                'Autorisierung',
                'erfolgreich',
                JTLLOG_LEVEL_NOTICE,
                'preparePaymentProcess',
                "header: " . print_r($agws_secupay_flex_http_header, 1) . "<br><br>" . $agws_secupay_flex_data,
                $agws_secupay_flex_antwort_json,
                $agws_secupay_flex_antwort
            );
            ...
            if ($agws_secupay_flex_antwort_json->status == "ok" && isset($agws_secupay_flex_antwort_json->data->hash)
                && $agws_secupay_flex_antwort_json->data->hash == $_SESSION['agws_secupay_flex_hash_tmp']) {
                //Logging + smarty
                $this->agws_secupay_flex_doLoggingCurl(
                    'Status',
                    'erfolgreich',
                ...
                header(
                    "location: bestellvorgang.php?editZahlungsart=1&AGWS_SECUPAY_ZA=RG&AGWS_SECUPAY_ERRORCODE="
                    . $agws_secupay_flex_antwort_json->status
                );
            }
        } else {
            //Logging + Redirect
    ...
    public function agws_secupay_flex_getPurpose()
    {
        if ($this->agws_secupay_flex_getLanguage() == "de_DE") {
            $agws_secupay_flex_purpose = utf8_encode(
                'Bestellung vom ' . date("d.m.Y") . ' bei ' . $this->agws_secupay_flex_getShopname()
            );
        } else {
            $agws_secupay_flex_purpose = utf8_encode(
                'Order from ' . date("Y-m-d") . ' by ' . $this->agws_secupay_flex_getShopname()
            );
        }
    ...
    public function agws_secupay_flex_getShopname()
    {
        ...
        if (strlen($this->oPlugin->oPluginEinstellungAssoc_arr['agws_secupay_flex_global_shopname']) > 1) {
            $agws_secupay_flex_shop_name = substr(
                $this->oPlugin->oPluginEinstellungAssoc_arr['agws_secupay_flex_global_shopname'],
                0,
                48
            );
        } else {
    ...
    public function agws_secupay_flex_getBasket($order)
    {
        ...
        foreach ($order_pos_arr as $order_pos) {
            if ($order_pos->nPosTyp == 1) {
                ...
                $agws_secupay_flex_basket[$ctr]->price          = utf8_encode(
                    round(($order_pos->fPreis + ($order_pos->fPreis / 100 * $order_pos->fMwSt)), 2) * 100
                );
                $agws_secupay_flex_basket[$ctr]->total          = utf8_encode(
                    round(
                        round(
                            ($order_pos->fPreis + ($order_pos->fPreis / 100 * $order_pos->fMwSt)) * $order_pos->nAnzahl,
                            4
                        ) * 100
                    )
                );
    ...
    public function agws_secupay_flex_getDeliveryAddress($order)
    {
        ...
        $agws_secupay_flex_deliveryaddress->firstname   = utf8_encode(
            html_entity_decode($order->Lieferadresse->cVorname, ENT_COMPAT | ENT_HTML401, "ISO8859-1")
        );
        $agws_secupay_flex_deliveryaddress->lastname    = utf8_encode(
            html_entity_decode($order->Lieferadresse->cNachname, ENT_COMPAT | ENT_HTML401, "ISO8859-1")
        );
        $agws_secupay_flex_deliveryaddress->company     = utf8_encode(
            html_entity_decode($order->Lieferadresse->cFirma, ENT_COMPAT | ENT_HTML401, "ISO8859-1")
        );
        $agws_secupay_flex_deliveryaddress->street      = utf8_encode(
            html_entity_decode($order->Lieferadresse->cStrasse, ENT_COMPAT | ENT_HTML401, "ISO8859-1")
        );
        $agws_secupay_flex_deliveryaddress->housenumber = utf8_encode(
            html_entity_decode($order->Lieferadresse->cHausnummer, ENT_COMPAT | ENT_HTML401, "ISO8859-1")
        );
        $agws_secupay_flex_deliveryaddress->zip         = utf8_encode(
            html_entity_decode($order->Lieferadresse->cPLZ, ENT_COMPAT | ENT_HTML401, "ISO8859-1")
        );
        $agws_secupay_flex_deliveryaddress->city        = utf8_encode(
            html_entity_decode($order->Lieferadresse->cOrt, ENT_COMPAT | ENT_HTML401, "ISO8859-1")
        );
        $agws_secupay_flex_deliveryaddress->country     = utf8_encode(
            html_entity_decode($order->Lieferadresse->cLand, ENT_COMPAT | ENT_HTML401, "ISO8859-1")
        );
    ...
    {
        ($this->secupayHelper->isShop4()) ? $agws_secupay_flex_useragent = 'JTL4-client V' . $this->oPlugin->nVersion
    ...
    {
        $this->secupayHelper->isShop4() ? $agws_url_shop = Shop::getURL() : $agws_url_shop = URL_SHOP;
        ...
        if (function_exists('curl_init')) {
            ...
            $agws_secupay_flex_logtext = 'secupay_flex - RG - function getCurlContent <br />
    ...
    {
        $agws_secupay_flex_logtext = 'secupay_flex - RG - function ' . $agws_function . ' <br />' . $agws_logbase . '-'
            . $agws_logstatus . ' <br />
		gesendete Daten: <br />' . print_r($agws_data, true) . ' <br/>
		---------------------- <br />
		Antwort secupay: <br />' . print_r($agws_json, true) . ' <br/>
		---------------------- <br />
		sonstige Daten: <br />' . print_r($agws_misc, true) . ' <br/>
		======================<br />';

        Jtllog::writeLog($agws_secupay_flex_logtext, $agws_loglevel);
        ZahlungsLog::add(
            $this->cModulId,
            'Autorisierung erfolgreich (' . $agws_json->status . ')',
    ...
    public function handleNotification($order, $agws_secupay_flex_paymentHash, $args)
    {
            ...
            if ($this->verifyNotification($order, $agws_secupay_flex_paymentHash, $args)) {
                ...
                if ($_SESSION['agws_secupay_flex_abwLA_flag'] == 1) {
                    $smarty->assign(
                        ...
                        $this->oPlugin->oPluginSprachvariableAssoc_arr['agws_secupay_flex_loc_global_abwLA_titel']
                    );
                    $smarty->assign(
                        'agws_secupay_flex_abwLA_text',
                        $this->oPlugin->oPluginSprachvariableAssoc_arr['agws_secupay_flex_loc_global_abwLA_text']
                    );
                    $agws_secupay_flex_Kommentar_abwLA = $smarty->fetch(
                        $this->oPlugin->cFrontendPfad . 'template/agws_secupay_flex_abwLA_text.tpl'
                    );
                }
                ...
                $this->secupayHelper->isShop4()
                    ? Shop::DB()
                          ->executeQuery(
                              "UPDATE tbestellung SET cKommentar = '" . $agws_secupay_flex_Kommentar
                              . "' WHERE kBestellung='" . $order->kBestellung . "'",
                              4
                          )
                    : $GLOBALS['DB']->executeQuery(
                    "UPDATE tbestellung SET cKommentar = '" . $agws_secupay_flex_Kommentar . "' WHERE kBestellung='"
                    . $order->kBestellung . "'",
                    4
                );
                ...
                $this->secupayHelper->isShop4()
                    ? Shop::DB()
					    ...
					    ('" . Shop::DB()
                                  ->realEscape($order->kBestellung) . "', '" . Shop::DB()
                                                                                   ->realEscape($_REQUEST['hash'])
                              . "',NOW(),'invoice')",
                              10
                          )
                    : $GLOBALS["DB"]->executeQuery(
                    "INSERT INTO xplugin_agws_secupay_flex_tsyslog
					(kBestellung, cHash, dSuccDat, cSecupayZA) VALUES
					('" . $GLOBALS["DB"]->realEscape($order->kBestellung) . "', '" . $GLOBALS["DB"]->realEscape(
                        $_REQUEST['hash']
                    ) . "', NOW(),'invoice')",
                    10
                );
        ...
        if (isset($_REQUEST['sh']) && isset($_REQUEST['agws_sp_url']) && $_REQUEST['agws_sp_url'] == 'push') {
            ...
            if ($this->verifyNotification(
                ...
                switch ((int) $this->oPlugin->oPluginEinstellungAssoc_arr[$this->cModulId
                . "_agws_secupay_flex_rg_due_design"]) {
                    case 0:
                    ...
                    case 1:
                        ...
                        $agws_secupay_flex_rg_due_text = $this->oPlugin->oPluginSprachvariableAssoc_arr['agws_secupay_flex_loc_invoice_due_text_days'];
                        $agws_secupay_flex_rg_due_days = $this->oPlugin->oPluginEinstellungAssoc_arr[$this->cModulId
                        . "_agws_secupay_flex_due_days"];
                        $agws_secupay_flex_rg_due_text = str_replace(
                            '|t|',
                            $agws_secupay_flex_rg_due_days,
                            $agws_secupay_flex_rg_due_text
                        );
                        break;
                    ...
                    case 2:
                        ...
                        $agws_secupay_flex_rg_due_days = $this->oPlugin->oPluginEinstellungAssoc_arr[$this->cModulId
                        . "_agws_secupay_flex_due_days"];
                        $agws_secupay_flex_rg_due_date = mktime(
                            0,
                            0,
                            0,
                            date("m"),
                            date("d") + $agws_secupay_flex_rg_due_days,
                            date("Y")
                        );
                        $agws_secupay_flex_rg_due_date = date("d.m.Y", $agws_secupay_flex_rg_due_date);
                        $agws_secupay_flex_rg_due_text = str_replace(
                            '|t|',
                            $agws_secupay_flex_rg_due_date,
                            $agws_secupay_flex_rg_due_text
                        );
                        break;
                }
                $smarty->assign(
                    'agws_secupay_flex_bank_ktonr',
                    $agws_secupay_flex_antwort_json->data->opt->transfer_payment_data->accountnumber
                );
                $smarty->assign(
                    'agws_secupay_flex_bank_blz',
                    $agws_secupay_flex_antwort_json->data->opt->transfer_payment_data->bankcode
                );
                $smarty->assign(
                    'agws_secupay_flex_bank_bank',
                    ($agws_secupay_flex_antwort_json->data->opt->transfer_payment_data->bankname)
                );
                $smarty->assign(
                    'agws_secupay_flex_bank_iban',
                    $agws_secupay_flex_antwort_json->data->opt->transfer_payment_data->iban
                );
                $smarty->assign(
                    'agws_secupay_flex_bank_bic',
                    $agws_secupay_flex_antwort_json->data->opt->transfer_payment_data->bic
                );
                $smarty->assign(
                    'agws_secupay_flex_rg_due_design',
                    $this->oPlugin->oPluginEinstellungAssoc_arr[$this->cModulId . "_agws_secupay_flex_rg_due_design"]
                );
                $smarty->assign('agws_secupay_flex_rg_due_text', utf8_encode($agws_secupay_flex_rg_due_text));
                ...
                $smarty->assign('agws_secupay_flex_bank_recipient_legal', ($agws_secupay_flex_antwort_json->data->opt->recipient_legal));

                $smarty->assign(
                    'agws_secupay_flex_bank_accountowner',
                    $agws_secupay_flex_antwort_json->data->opt->transfer_payment_data->accountowner
                );

                $agws_secupay_flex_Kommentar_BankVerbindung = utf8_decode($smarty->fetch(
                    $this->oPlugin->cFrontendPfad . 'template/agws_secupay_flex_bank_text_' . $_SESSION['cISOSprache']
                    . '.tpl'
                ));
                $this->secupayHelper->isShop4()
                    ? $oResTmp = Shop::DB()
                ...
                Jtllog::writeLog(
                    'RG selsect Bankverbindung' . $agws_secupay_flex_Kommentar_BankVerbindung
                    . ' hat kBestellung. Modul ' . $order->Zahlungsart->cModulId . ' wird aufgerufen',
                    JTLLOG_LEVEL_DEBUG,
                    false,
                    'Notify'
                );
                ...
                Jtllog::writeLog(
                    'RG selsect kommnetar' . $agws_secupay_flex_Kommentar . ' hat kBestellung. Modul '
                ...
                Jtllog::writeLog(
                    'RG selsect kommnetar2' . $agws_secupay_flex_Kommentar . ' hat kBestellung. Modul '
                    . $order->Zahlungsart->cModulId . ' wird aufgerufen',
                    JTLLOG_LEVEL_DEBUG,
                    false,
                    'Notify'
                );
                if ($this->oPlugin->oPluginEinstellungAssoc_arr[$this->cModulId . "_agws_secupay_flex_rg_comment"]
                    == 2) {
                    $this->secupayHelper->isShop4()
                        ? Shop::DB()
                              ->executeQuery(
                                  "UPDATE tbestellung SET cPUIZahlungsdaten = '"
                                  . $agws_secupay_flex_Kommentar_BankVerbindung . "' WHERE kBestellung='"
                                  . $order->kBestellung . "'",
                                  4
                              )
                        : $GLOBALS['DB']->executeQuery(
                        "UPDATE tbestellung SET cPUIZahlungsdaten = '" . $agws_secupay_flex_Kommentar_BankVerbindung
                        . "' WHERE kBestellung='" . $order->kBestellung . "'",
                        4
                    );
                } else {
                ...
                $this->agws_secupay_flex_sendConfirmationMail(
                    ...
                    $agws_secupay_flex_rg_due_text,
                    utf8_decode(
                        $agws_secupay_flex_antwort_json->data->opt->recipient_legal
                    ),
                    $agws_secupay_flex_antwort_json->data->opt->transfer_payment_data->accountowner,
                    $agws_secupay_flex_antwort_json->data->opt->transfer_payment_data->accountnumber,
                    $agws_secupay_flex_antwort_json->data->opt->transfer_payment_data->bankcode,
                    ...
                    $agws_secupay_flex_antwort_json->data->opt->transfer_payment_data->iban,
                    $agws_secupay_flex_antwort_json->data->opt->transfer_payment_data->bic,
                    $agws_secupay_flex_antwort_json->data->opt->transfer_payment_data->purpose,
                    $agws_secupay_flex_antwort_json->data->opt->payment_link,
                    $agws_secupay_flex_antwort_json->data->opt->payment_qr_image_url
                );
                ...
                $this->secupayHelper->isShop4()
                    ? Shop::DB()
                          ->executeQuery(
                              "UPDATE xplugin_agws_secupay_flex_tsyslog SET cHash='" . Shop::DB()
                                                                                           ->realEscape(
                                                                                               $_REQUEST['hash']
                                                                                           ) . "',cTACode='" . Shop::DB(
                              )
                                                                                                                   ->realEscape(
                                                                                                                       $agws_secupay_flex_antwort_json->data->trans_id
                                                                                                                   )
                              . "',kAmountSecupay='" . Shop::DB()
                                                           ->realEscape($agws_secupay_flex_antwort_json->data->amount)
                              . "',dPushDat=now() WHERE kBestellung='" . $order->kBestellung . "'",
                              4
                          )
                    : $GLOBALS["DB"]->executeQuery(
                    "UPDATE xplugin_agws_secupay_flex_tsyslog SET cHash='" . $GLOBALS["DB"]->realEscape(
                        $_REQUEST['hash']
                    ) . "',cTACode='" . $GLOBALS["DB"]->realEscape($agws_secupay_flex_antwort_json->data->trans_id)
                    . "',kAmountSecupay='" . $GLOBALS["DB"]->realEscape($agws_secupay_flex_antwort_json->data->amount)
                    . "',dPushDat=now() WHERE kBestellung='" . $order->kBestellung . "'",
                    4
                );
                ...
                $agws_secupay_flex_logtext = 'secupay_flex - RG - function handleNotification <br />
                ...
                $agws_secupay_flex_ackreq = 'ack=Disapproved&error=Verifikation_fehlerhaft_oder_Multi_Push&'
    ...
    public function verifyNotification($order, $agws_secupay_flex_paymentHash, $args, $agws_secupay_flex_antwort_json = "")
    {
        switch ($_REQUEST['agws_sp_url']) {
            case "succ":
                ...
                $agws_secupay_flex_data        = json_encode($agws_secupay_flex_daten);
                $agws_secupay_flex_http_header = $this->agws_secupay_flex_getHttpHeader(
                    strlen($agws_secupay_flex_data)
                );
                ...
                do {
                    ...
                    $agws_secupay_flex_antwort_json = json_decode($agws_secupay_flex_antwort);
                    $status                         = in_array(
                        $agws_secupay_flex_antwort_json->data->status,
                        array('accepted', 'authorized')
                    );
                ...
                if ($status == true
                    && (($_REQUEST['sh'] == $agws_secupay_flex_paymentHash)
                        || ($_REQUEST['sh'] == "_" . $agws_secupay_flex_paymentHash))) {
                    $agws_secupay_flex_logtext = 'secupay_flex - RG - function verifyNotification <br />
            ...
            case "push":
                ...
                Jtllog::writeLog(print_r($_REQUEST, true), JTLLOG_LEVEL_DEBUG);
                if (($_REQUEST['sh'] == $agws_secupay_flex_paymentHash)
                    || ($_REQUEST['sh'] == "_" . $agws_secupay_flex_paymentHash)
                    ...
                    $this->secupayHelper->isShop4()
                        ? $oNotifyDate = Shop::DB()
                                             ->executeQuery(
                                                 "SELECT dNotify FROM tzahlungsession WHERE kBestellung = " . intval(
                                                     $order->kBestellung
                                                 ),
                                                 8
                                             )
                    ...
                    if (count($oNotifyDate) > 0 && !empty($oNotifyDate['dNotify'])) {
                        ...
                        ZahlungsLog::add(
                            $this->cModulId,
                            'mehrfache Pushmitteilung erhalten',
                            $agws_secupay_flex_logtext,
                            JTLLOG_LEVEL_NOTICE
                        );
                    ...
                    if ($agws_secupay_flex_antwort_json->status == "ok"
                        ...
                        && round($agws_secupay_flex_antwort_json->data->amount) == round(
                            (round($order->fGesamtsumme, 2) * 100)
                        )) {
                        $agws_secupay_flex_antwort_json->status == "ok" ? $status_flag = "Status OK"
                            : $status_flag = "Status FAIL";

                        isset($agws_secupay_flex_antwort_json->data->hash)
                        && $agws_secupay_flex_antwort_json->data->hash == $_REQUEST['hash'] ? $hash_flag = "hash OK"
                            : $hash_flag = "hash FAIL";

                        round($agws_secupay_flex_antwort_json->data->amount) == round(
                            (round($order->fGesamtsumme, 2) * 100)
                        ) ? $sum_flag = "Gesamtsumme OK" : $sum_flag = "Gesamtsumme FAIL";

                        $sum_diff = round($agws_secupay_flex_antwort_json->data->amount) - round(
                                (round($order->fGesamtsumme, 2) * 100)
                            );
                        ...
                        ZahlungsLog::add(
                            $this->cModulId,
                            'multiVar-Vergleich erfolgreich(0)',
                            $agws_secupay_flex_logtext,
                            JTLLOG_LEVEL_NOTICE
                        );
                        ...
                        $agws_secupay_flex_antwort_json->status != "ok" ? $status_flag = "Status FAIL"
                            : $status_flag = "Status OK";

                        isset($agws_secupay_flex_antwort_json->data->hash)
                        && $agws_secupay_flex_antwort_json->data->hash != $_REQUEST['hash'] ? $hash_flag = "hash FAIL"
                            : $hash_flag = "hash OK";

                        round($agws_secupay_flex_antwort_json->data->amount) != round(
                            (round($order->fGesamtsumme, 2) * 100)
                        ) ? $sum_flag = "Gesamtsumme FAIL" : $sum_flag = "Gesamtsumme OK";
                        ...
                        ZahlungsLog::add(
                            $this->cModulId,
                            'multiVar-Vergleich fehlerhaft(0)',
                            $agws_secupay_flex_logtext,
                            JTLLOG_LEVEL_ERROR
                        );
                    ...
                    ZahlungsLog::add(
                        $this->cModulId,
                        'multiVar-Vergleich fehlerhaft(1)',
                        $agws_secupay_flex_logtext,
                        JTLLOG_LEVEL_ERROR
                    );
    ...
    public function isValid($customer, $cart)
    {
        ...
        if ($this->getSetting('min_bestellungen') > 0) {
            if (isset($customer->kKunde) && $customer->kKunde > 0) {
                ...
                if ($count < $this->getSetting('min_bestellungen')) {
                    ZahlungsLog::add(
                        $this->moduleID,
                        'Bestellanzahl ' . $count . ' ist kleiner als der Mindestanzahl von ' . $this->getSetting(
                            'min_bestellungen'
                        ),
                        null,
                        LOGLEVEL_NOTICE
        ...
        if ($this->getSetting('min') > 0 && $cart->gibGesamtsummeWaren(1) <= $this->getSetting('min')) {
            ZahlungsLog::add(
                $this->moduleID,
                'Bestellwert ' . $cart->gibGesamtsummeWaren(1) . ' ist kleiner als der Mindestbestellwert von '
                . $this->getSetting('min'),
                null,
                LOGLEVEL_NOTICE
            );
        ...
        if ($this->getSetting('max') > 0 && $cart->gibGesamtsummeWaren(1) >= $this->getSetting('max')) {
            ZahlungsLog::add(
                $this->moduleID,
                'Bestellwert ' . $cart->gibGesamtsummeWaren(1) . ' ist groesser als der maximale Bestellwert von '
                . $this->getSetting('max'),
                null,
                LOGLEVEL_NOTICE
            );
        ...
        if ($agws_secupay_flex_antwort_json->status == "ok"
            && in_array(
                "invoice",
                $agws_secupay_flex_antwort_json->data
            )) {
            $this->agws_secupay_flex_doLoggingCurl(
                'Statusabfrage',
                'erfolgreich',
