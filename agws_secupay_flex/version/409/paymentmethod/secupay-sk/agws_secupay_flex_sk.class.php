...
/**
 * Class agws_secupay_flex_sk
 */
class agws_secupay_flex_sk extends PaymentMethod
{
    /**
     * agws_secupay_flex_sk constructor.
     *
     * @param     $moduleID
    ...
    public function init($nAgainCheckout = 0)
    {
        ...
        $this->name    = 'agws-secupay-sk';
        $this->caption = 'agws-secupay-sk';
    }
    /**
    ...
    public function preparePaymentProcess($order)
    {
        ...
        if ($agws_secupay_flex_antwort_json->status == "ok" && isset($agws_secupay_flex_antwort_json->data->iframe_url)
            ...
            if ($agws_secupay_flex_antwort_json->status == "ok" && isset($agws_secupay_flex_antwort_json->data->hash)
                ...
                header(
                    "location: bestellvorgang.php?editZahlungsart=1&AGWS_SECUPAY_ZA=SK&AGWS_SECUPAY_ERRORCODE="
    ...
    public function agws_secupay_flex_getCurlContent($agws_linkaction, $agws_httpheader, $agws_data = null)
    {
        ...
        if (function_exists('curl_init')) {
            ...
            $agws_secupay_flex_logtext = 'secupay_flex - SK - function getCurlContent <br />
    ...
    {
        $agws_secupay_flex_logtext = 'secupay_flex - SK - function ' . $agws_function . ' <br />' . $agws_logbase . '-'
            . $agws_logstatus . ' <br />
    ...
    public function handleNotification($order, $agws_secupay_flex_paymentHash, $args)
    {
        ...
        if (isset($_REQUEST['sh']) && isset($_REQUEST['agws_sp_url']) && $_REQUEST['agws_sp_url'] == 'push') {
            ...
            if ($this->verifyNotification(
                ...
                $agws_secupay_flex_zweck        = "TA " . $agws_secupay_flex_antwort_json->data->trans_id . " DT "
                    . date("Ymd", $agws_secupay_flex_dt_timestamp);

                $smarty->assign('agws_secupay_flex_zweck', $agws_secupay_flex_zweck);
                $agws_secupay_flex_Kommentar_Zweck = $smarty->fetch(
                    $this->oPlugin->cFrontendPfad . 'template/agws_secupay_flex_zweck_text_' . $_SESSION['cISOSprache']
                    . '.tpl'
                );
                ...
                if ($this->oPlugin->oPluginEinstellungAssoc_arr[$this->cModulId . "_agws_secupay_flex_sk_comment"]
                    == 2) {
                    $this->secupayHelper->isShop4()
                        ? Shop::DB()
                              ->executeQuery(
                                  "UPDATE tbestellung SET cPUIZahlungsdaten = '" . $agws_secupay_flex_Kommentar
                                  . "' WHERE kBestellung='" . $order->kBestellung . "'",
                                  4
                              )
                        : $GLOBALS['DB']->executeQuery(
                ...
                $agws_secupay_flex_logtext = 'secupay_flex - SK - function handleNotification <br />
        ...
        switch ($_REQUEST['agws_sp_url']) {
            case "succ":
                ...
                if ($status == true
                    ...
                    $agws_secupay_flex_logtext = 'secupay_flex - SK - function verifyNotification <br />
     ...
     * @param $kBestellung
     */
    public function agws_secupay_flex_sendConfirmationMail($kBestellung)
    {
