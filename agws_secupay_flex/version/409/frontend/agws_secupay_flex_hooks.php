...
if ($helper->gibSeiten__Typ() == PAGE_BESTELLVORGANG) {
    if (!is_null($_SESSION['Bestellung']->kLieferadresse) && $_SESSION['Bestellung']->kLieferadresse != '0'
        && $oPlugin->oPluginEinstellungAssoc_arr['agws_secupay_flex_global_hinweisflag'] == 1) {
        $smarty->assign(
            'agws_secupay_flex_abwLA_titel',
            $oPlugin->oPluginSprachvariableAssoc_arr['agws_secupay_flex_loc_global_abwLA_titel']
        );
        $smarty->assign(
            'agws_secupay_flex_abwLA_text',
            $oPlugin->oPluginSprachvariableAssoc_arr['agws_secupay_flex_loc_global_abwLA_text']
        );
        $agws_secupay_flex_abwLA                  = $smarty->fetch(
            $oPlugin->cFrontendPfad . 'template/agws_secupay_flex_abwLA_html.tpl'
        );
    ...
    switch ($oPlugin->nCalledHook) {
        case HOOK_SMARTY_OUTPUTFILTER:  //Hook140
            ...
            if (($smarty->get_template_vars('step') == "Zahlung")
                && ($oPlugin->oPluginEinstellungAssoc_arr['agws_secupay_flex_global_hinweisflag'] == 1
                    || $oPlugin->oPluginEinstellungAssoc_arr['agws_secupay_flex_global_hinweisflag'] == 2)) {
                //$agws_secupay_flex_cModulId_arr nur für Lastschrift und Rechnungskauf, da bei Kreditkarte abweichende LA zulässig
                $agws_secupay_flex_cModulId_arr = array(
                    "kPlugin_" . $oPlugin->kPlugin . "_secupaycredit",
                    "kPlugin_" . $oPlugin->kPlugin . "_secupaydebit",
                    "kPlugin_" . $oPlugin->kPlugin . "_secupayinvoice" . $oPlugin->kPlugin . "_secupayprepay"
                    . $oPlugin->kPlugin . "_secupaysofort"
                );

                for ($i = 0; $i < count($oPlugin->oPluginZahlungsmethode_arr); $i++) {
                    if (($_SESSION['Bestellung']->kLieferadresse != 0)
                        && ($oPlugin->oPluginEinstellungAssoc_arr['agws_secupay_flex_global_hinweisflag'] == 2)
                        && in_array(
                            $oPlugin->oPluginZahlungsmethode_arr[$i]->cModulId,
                            $agws_secupay_flex_cModulId_arr
                        )) {
                        pq('#' . $oPlugin->oPluginZahlungsmethode_arr[$i]->cModulId . '')->addClass('hidden');
                    } elseif (($_SESSION['Bestellung']->kLieferadresse != 0)
                        && ($oPlugin->oPluginEinstellungAssoc_arr['agws_secupay_flex_global_hinweisflag'] == 1)
                        && in_array(
                            $oPlugin->oPluginZahlungsmethode_arr[$i]->cModulId,
                            $agws_secupay_flex_cModulId_arr
        ...
        case HOOK_BESTELLVORGANG_PAGE:  //Hook19
            if ($_GET['AGWS_SECUPAY_ERRORMSG']) {
                $smarty->assign(
                    'cFehler',
                    "<small>" . $_GET['AGWS_SECUPAY_ERRORMSG'] . " (" . $_GET['AGWS_SECUPAY_ERRORCODE'] . ")</small>"
                );
            }
            break;
        default:
...
} elseif (($helper->gibSeiten__Typ() == PAGE_UNBEKANNT && isset($_GET['uid']) && $_GET['uid'] != "")
    || ($helper->gibSeiten__Typ() == PAGE_BESTELLSTATUS)) {
    switch ($oPlugin->nCalledHook) {
        case HOOK_SMARTY_OUTPUTFILTER:  //Hook140
            $agws_secupay_flex_status_ordercomplete = $smarty->fetch(
                $oPlugin->cFrontendPfad . 'template/agws_secupay_flex_status_ordercomplete.tpl'
            );
            ...
            $agws_secupay_flex_cModulId_arr = array(
                "kPlugin_" . $oPlugin->kPlugin . "_secupaycredit",
                "kPlugin_" . $oPlugin->kPlugin . "_secupaydebit",
                "kPlugin_" . $oPlugin->kPlugin . "_secupayinvoice"
            );

            if (in_array($agws_secupay_flex_oBestellung->Zahlungsart->cModulId, $agws_secupay_flex_cModulId_arr)
                && ($agws_secupay_flex_oBestellung->dBezahltDatum == "0000-00-00")) {
                ...
                if ($helper->isShop4()) {
                    ...
                    $cTmp = '<fieldset class="resize" style="height: 30px;"><legend>' . $helper->gib__Wert(
                            'paymentOptions',
                            'global'
                        ) . ': ' . $agws_secupay_flex_oBestellung->cZahlungsartName . '</legend><span>'
                        . $agws_secupay_flex_pqText . '</span></fieldset>';
                    pq('div.container:eq(1) ul li:eq(1) div fieldset')->remove();
                    pq('div.container:eq(1) ul li:eq(1) div')->append($cTmp);
                }
            } elseif (in_array($agws_secupay_flex_oBestellung->Zahlungsart->cModulId, $agws_secupay_flex_cModulId_arr)
                && ($agws_secupay_flex_oBestellung->dBezahltDatum != "0000-00-00")) {
        ...
        switch ($oPlugin->nCalledHook) {
            case HOOK_SMARTY_OUTPUTFILTER:  //Hook140
                ...
                $agws_secupay_flex_cModulId_arr = array(
                    ...
                    "kPlugin_" . $oPlugin->kPlugin . "_secupaydebit",
                    "kPlugin_" . $oPlugin->kPlugin . "_secupayinvoice",
                    "kPlugin_" . $oPlugin->kPlugin . "_secupayprepay",
                    "kPlugin_" . $oPlugin->kPlugin . "_secupaysofort"
                );
                if (in_array($agws_secupay_flex_oBestellung->Zahlungsart->cModulId, $agws_secupay_flex_cModulId_arr)
                    && ($agws_secupay_flex_oBestellung->dBezahltDatum == "0000-00-00")) {
                    $agws_secupay_flex_pqText = $oPlugin->oPluginSprachvariableAssoc_arr['agws_secupay_flex_loc_global_auth_pending'];
                    if ($helper->isShop4()) {
                        ...
                        $cTmp = '<fieldset class="resize" style="height: 50px;"><legend>' . $helper->gib__Wert(
                                'paymentOptions',
                                'global'
                            ) . ': ' . $agws_secupay_flex_oBestellung->cZahlungsartName . '</legend><span>'
                            . $agws_secupay_flex_pqText . '</span></fieldset>';
                        ...
                        $agws_secupay_flex_oBestellung->Zahlungsart->cModulId,
                        $agws_secupay_flex_cModulId_arr
                    )
                    && ($agws_secupay_flex_oBestellung->dBezahltDatum != "0000-00-00")) {
                    $agws_secupay_flex_pqText = $oPlugin->oPluginSprachvariableAssoc_arr['agws_secupay_flex_loc_global_auth_accept'];
...
if ($oPlugin->nCalledHook == HOOK_BESTELLUNGEN_XML_BEARBEITESET) {
    ...
    $sql = "SELECT tbestellung.kBestellung, tbestellung.cTracking,tbestellung.cLogistiker,tbestellung.dVersandDatum,tbestellung.cStatus,tbestellung.cBestellNr, xplugin_agws_secupay_flex_tsyslog.kLogId, xplugin_agws_secupay_flex_tsyslog.dVersandDat, xplugin_agws_secupay_flex_tsyslog.cHash, xplugin_agws_secupay_flex_tsyslog.cSecupayZA
			FROM tbestellung
    ...
    ($helper->isShop4()) ? $oBestellungVersand = Shop::DB()
                                                     ->executeQuery($sql, 1)
        : $oBestellungVersand = $GLOBALS['DB']->executeQuery($sql, 1);

    if (!is_null($oBestellungVersand) && $oBestellungVersand->cStatus == BESTELLUNG_STATUS_VERSANDT
        && $oBestellungVersand->dVersandDat == "0000-00-00 00:00:00") {
        Jtllog::writeLog(
            "secupay: Versand für secupay-Bestellung (" . $oBestellungVersand->kBestellung . ")",
            JTLLOG_LEVEL_NOTICE
        );

        if ($oBestellungVersand->cSecupayZA === null || $oBestellungVersand->cSecupayZA == "invoice"
            || $oBestellungVersand->cSecupayZA == "debit"
            || $oBestellungVersand->cSecupayZA == "creditcard"
            || $oBestellungVersand->cSecupayZA == "prepay"
            || $oBestellungVersand->cSecupayZA == "sofort") {
            Jtllog::writeLog("secupay: Versand für Rechnungskauf - löse capture aus", JTLLOG_LEVEL_NOTICE);

            $agws_secupay_flex_curlcontent = "";

            if (function_exists('curl_init')) {
                ($helper->isShop4()) ? $agws_secupay_flex_useragent = 'JTL4-client V' . $oPlugin->nVersion
                ...
                $agws_secupay_flex_daten = array(
                    'data' => array(
                        ...
                        'hash'           => $oBestellungVersand->cHash,
                        'tracking'       => array(
                            'provider' => $oBestellungVersand->cLogistiker,
                            'number'   => $oBestellungVersand->cTracking
                        ),
                        'invoice_number' => $oBestellungVersand->cBestellNr
                    )
                );
                if ($oBestellungVersand->cSecupayZA == "invoice") {
                    $agws_secupay_flex_CURL_URL = "https://api.secupay.ag/payment/capture/";
                } else {
                    $agws_secupay_flex_CURL_URL = "https://api.secupay.ag/payment/adddata/";
                }
                ...
                $agws_secupay_flex_logtext = 'secupay_flex - hook - setzeVersand <br />
                ...
                Jtllog::writeLog(
                    "secupay: capture-Antwort: " . print_r($agws_secupay_flex_antwort, 1),
                    JTLLOG_LEVEL_NOTICE
                );
            }
        }
        ...
        ($helper->isShop4())
            ? $queryResult = Shop::DB()
                                 ->updateRow(
                                     "xplugin_agws_secupay_flex_tsyslog",
                                     "kLogId",
                                     $oBestellungVersand->kLogId,
                                     $queryResultInsert
                                 ) : $queryResult = $GLOBALS["DB"]->updateRow(
            "xplugin_agws_secupay_flex_tsyslog",
            "kLogId",
            $oBestellungVersand->kLogId,
            $queryResultInsert
